...
class CVRandomRotation(object):
    def __init__(this, degrees=15,border_mode=cv2.BORDER_REPLICATE):
    ...
    def __call__(this, img):
        angle = this.get_params(this.degrees)
        ...
        return cv2.warpAffine(img, M, (dst_w, dst_h), flags=flags, borderMode=this.border_mode,borderValue=[127,127,127]);


class CVRandomAffine(object):
    def __init__(this, degrees, translate=None, scale=None, shear=None,border_mode=cv2.BORDER_REPLICATE):
        assert isinstance(degrees, numbers.Number), "degree should be a single number."
        assert degrees >= 0, "degree must be positive."
        this.degrees = degrees
        this.border_mode = border_mode;
    ...
    def __call__(this, img):
        src_h, src_w = img.shape[:2]
        angle, translate, scale, shear = this.get_params(
            this.degrees, this.translate, this.scale, this.shear, src_h)

        M = this._get_inverse_affine_matrix((src_w / 2, src_h / 2), angle, (0, 0), scale, shear)
...
class CVRandomPerspective(object):
    def __init__(this, distortion=0.5,border_mode=cv2.BORDER_REPLICATE):
        this.distortion = distortion;
        this.border_mode=border_mode

    def get_params(this, width, height, distortion):
    ...
    def __call__(this, img):
        ...
        startpoints, endpoints = this.get_params(width, height, this.distortion)
        ...
        flags = get_interpolation()
        img = cv2.warpPerspective(img, M, (max_x, max_y), flags=flags, borderMode=this.border_mode,borderValue=[127,127,127]);
        img = img[min_y:, min_x:]
        return img
...
class CVGeometry(object):
    def __init__(self, degrees=15, translate=(0.3, 0.3), scale=(0.5, 2.),
                 shear=(45, 15), distortion=0.5, p=0.5,border_mode=cv2.BORDER_REPLICATE):
        self.p = p
        type_p = random.random()
        print("typep called");
        if type_p < 0.33:
            self.transforms = CVRandomRotation(degrees=degrees,border_mode=border_mode)
        elif type_p < 0.66:
            self.transforms = CVRandomAffine(degrees=degrees, translate=translate, scale=scale, shear=shear,border_mode=border_mode)
        else:
            self.transforms = CVRandomPerspective(distortion=distortion,border_mode=border_mode)
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            return img
class CVGeometryFixed(object):
    def __init__(this, degrees=15, translate=(0.3, 0.3), scale=(0.5, 2.),
                 shear=(45, 15), distortion=0.5, p=0.5,border_mode=cv2.BORDER_REPLICATE):
        this.p = p
        this.rcore=CVRandomRotation(degrees=degrees,border_mode=border_mode);
        this.acore=CVRandomAffine(degrees=degrees, translate=translate, scale=scale, shear=shear,border_mode=border_mode);
        this.pcore=CVRandomPerspective(distortion=distortion,border_mode=border_mode);


    def __call__(this, img):

        if random.random() < this.p:
            type_p = random.random()
            if type_p < 0.33:
                transforms = this.rcore;
            elif type_p < 0.66:
                transforms = this.acore;
            else:
                transforms = this.pcore;
            img = np.array(img)
            return Image.fromarray(transforms(img))
        else:
            return img
...
def get_abi_aug(border_mode=cv2.BORDER_REPLICATE):
    return transforms.Compose([
                CVGeometry(degrees=45, translate=(0.0, 0.0), scale=(0.5, 2.), shear=(45, 15), distortion=0.5, p=0.5,border_mode=border_mode),
                CVDeterioration(var=20, degrees=6, factor=4, p=0.25),
                CVColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1, p=0.25)
            ]);

def get_abi_aug_fixed(border_mode=cv2.BORDER_REPLICATE):
    return transforms.Compose([
                CVGeometryFixed(degrees=45, translate=(0.0, 0.0), scale=(0.5, 2.), shear=(45, 15), distortion=0.5, p=0.5,border_mode=border_mode),
                CVDeterioration(var=20, degrees=6, factor=4, p=0.25),
                CVColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1, p=0.25)
            ]);

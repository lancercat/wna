...
class lmdb_wrapper:
    ...
    def __init__(this,lmdb_dir,start=0):
        this.start=start;
        this.root=lmdb_dir;
        os.makedirs(lmdb_dir, exist_ok=True)

        this.db=lmdb.open(lmdb_dir,map_size=int(1e12));
        this.load=0;
    ...
    def adddata_kv(this, ikvdict, tkvdict, rkvdict):
        ...
        tks=[];
        for ik in ikvdict:
            imageKey = (ik + '-%09d' %  (this.load+this.start)).encode();
            iks.append(imageKey);
            this.txn.put(imageKey, cv2.imencode(".png", ikvdict[ik])[1]);
        for rk in rkvdict:
            rawKey = (rk + '-%09d' %  (this.load+this.start)).encode();
            rks.append(rawKey);
            this.txn.put(rawKey, rkvdict[rk]);
        for tk in tkvdict:
            tKey = (tk + '-%09d'%  (this.load+this.start)).encode() ;
            tks.append(tKey);
